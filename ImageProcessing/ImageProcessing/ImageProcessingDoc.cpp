	...
	if (dlg.DoModal() == IDOK) { // DoModal 대화상자의 활성화 여부
		...
		for (i = 0; i < m_height; i++) {
			for (j = 0; j < m_width; j++) {
				...
				m_OutputImage[(i * dlg.m_UpSampleRate * m_Re_width + dlg.m_UpSampleRate * j) + 1]
					= m_InputImage[i * m_width + j];
		...
		//3. 인접한 인덱스의 평균으로 채우는 방법
		//for (i = 0; i < m_height; i++) { //행으로 생각
		//	for (j = 0; j < m_width; j++) { // 열로 생각
		//		m_OutputImage[i * dlg.m_UpSampleRate * m_Re_width + dlg.m_UpSampleRate * j]
		//			= m_InputImage[i * m_width + j];
		//	} // 재배치하여 영상 확대
		//}
		//for (int x = 0; x < m_size; x = x+ dlg.m_UpSampleRate) {
		//	m_OutputImage[x+1] = (m_OutputImage[x] + m_OutputImage[x + dlg.m_UpSampleRate]) / 2;
		//}
	}
}
...
void CImageProcessingDoc::OnEndInSearch()
{
	...
	for (i = 0; i < m_size; i++) {
		...
		if (m_InputImage[i] <= LOW) {
			m_OutputImage[i] = 0;
		}

		// 원본 영상의 최대값보다 큰 값은 255
		else if (m_InputImage[i] >= HIGH) {
			m_OutputImage[i] = 255;
		}
		else
			m_OutputImage[i] = (unsigned char)((m_InputImage[i] -
	...
	for (i = 0; i < m_height; i++) {
		for (j = 0; j < m_height; j++) {
			...
			if (newH < 0 || newH >= m_height) {
				//회전된 자표가 출력 영상을 위한 배열 값을 넘어갈때
				Value = 0;
			}
			else if (newW < 0 || newW >= m_width) {
				//회전된 자표가 출력 영상을 위한 뱌열 값을 넘어갈때
				Value = 0;
			}
			else {
